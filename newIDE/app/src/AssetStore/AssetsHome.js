...
export const AssetsHome = React.forwardRef<Props, AssetsHomeInterface>(
  (
    ...
    const { receivedAssetPacks, receivedGameTemplates } = useContext(
      AuthenticatedUserContext
    );

    const scrollView = useRef<?ScrollViewInterface>(null);
    ...
    // Fetching NFTs using the context
    ...
    return (
      ...
      >
        ...
        <Line alignItems="center">
          {isLoading ? (
            <div style={styles.loaderContainer}>
              <PlaceHolderLoader />{' '}
              {/* Display PlaceHolderLoader in the center of the screen */}
            </div>
          ) : (
        ...
        {/* Additional sections like categories, promotions, bundles, etc., can go here */}
      </ScrollView>
    );
  }
);
